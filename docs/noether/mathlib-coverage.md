...
Bearbeitung. Its 35 claim/definition records are mapped in
...
| finite module reduction, Satz VII | finite-free and Smith-normal-form APIs; monic polynomial division; submodule sums and disjointness; `LinearMap.quotientInfEquivSupQuotient`; polynomial function extensionality and finite products | equations (21)--(23) are promoted for a supplied regular divisor: exact bounded representatives, finite coefficient vectors, the bounded-part/principal-tail decomposition, finite free quotient, exact containing-ideal image, the abstract common-tail quotient, its concrete regular-pair specialization, and the cutoff-one ground-ideal quotient counterpart. The first `ξ`/`ζ` coordinate model now realizes the cutoff-one ground/original pair and explicitly connects its coordinate quotient to the transported ideal quotient and bounded model under a supplied regular member; this bridge is `FORMALIZED_GAP`. Later stages, line 13162's ground-module characterization, and auxiliary-choice independence remain `GAP_CANDIDATE`. |
| equation (24), stage resultants, and elementary-divisor forms, Satz VII and Definition VI | Smith bases and coefficients; `Submodule.quotientEquivPiSpan`; determinants | a generic full-rank finite-free PID `NEW_PACKAGING` now exposes diagonal bases and coefficients, cyclic quotient factors, common-tail composition, and the selected-basis determinant product. It is not the historical localized `G₁*/M₁*` instance, a divisibility-ordered canonical elementary-divisor theorem, or the infinite-tail nonunit-factor conclusion. Those hypotheses, determinant norms, stage-resultant construction, and primitive normalization remain central `GAP_CANDIDATE`s; this is not ordinary `Polynomial.resultant`. |
