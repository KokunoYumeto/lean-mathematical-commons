...
| Source layer | Pinned coverage | State |
|---|---|---|
| ground modules, module quotients, and transition norms, Definitions I--III and Sätze I--V | submodule saturation, module torsion and rank-nullity; finite-free modules, Smith normal form, determinant norms, colon ideals | Definition I's formula-(2) cancellation and formula-(4) saturation, Satz I's least-ground-module construction, its same-rank consequence, and the equation-(24) first-stage scalar-colon quotients are `FORMALIZED_GAP`. The exact historical divisor phrasing, transition norm, and reconstruction theorems in Definitions II--III and Sätze II--V remain `GAP_CANDIDATE`. |
...
| equation (24), stage resultants, and elementary-divisor forms, Satz VII and Definition VI | Smith bases and coefficients; `Submodule.quotientEquivPiSpan`; `Submodule.colon`; determinants; polynomial localization | The actual finite localized cutoff-one pair over `K[x₂]` has selected Smith bases, coefficients, diagonal relation, cyclic quotient decomposition, and selected-basis determinant product. A freely adjoined arbitrary Finsupp common tail, including the countable `ζ` specialization, has unit tail coefficients and cancels exactly back to that finite quotient. The displayed module slashes are formalized as scalar colon ideals: adjoining one `ηᵢ` gives `(eᵢ)` unconditionally, while quotienting by the whole ground module is the infimum of the remaining principal ideals and becomes one `(eᵢ)` only under an explicit greatest-remaining-divisor hypothesis. Generic helpers are `NEW_PACKAGING`; the 20 source-instantiated scalar declarations are `FORMALIZED_GAP`. Canonical divisibility ordering, normalization, choice independence, identification of the free tail with an independently constructed historical unbounded `ζ` module, determinant norms, stage resultants, and primitive normalization remain central gaps; this is not ordinary `Polynomial.resultant`. |
