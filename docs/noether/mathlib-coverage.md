...
Bearbeitung. Its 47 machine-readable coverage records are mapped in
...
| equation (24), stage resultants, and elementary-divisor forms, Satz VII and Definition VI | Smith bases and coefficients; `Submodule.quotientEquivPiSpan`; `Submodule.colon`; module annihilators; finite ideal products and powers; determinants; polynomial localization | The actual finite localized cutoff-one pair over `K[x₂]` has selected Smith bases, coefficients, diagonal relation, cyclic quotient decomposition, and selected-basis determinant product. A freely adjoined arbitrary Finsupp common tail, including the countable `ζ` specialization, has unit tail coefficients and cancels exactly back to that finite quotient. The displayed module slashes are formalized as scalar colon ideals: adjoining one `ηᵢ` gives `(eᵢ)` unconditionally, while quotienting by the whole ground module is the infimum of the remaining principal ideals and becomes one `(eᵢ)` only under an explicit greatest-remaining-divisor hypothesis. The finite quotient annihilator is now identified intrinsically with `⋂ᵢ(eᵢ)` and with the denominator colon. For `D = ∏ᵢeᵢ` and `A` that annihilator, `(D) ≤ A` and `A^ρ ≤ (D)`; the principal generators satisfy both divisibilities and have equal radicals. Generic helpers are `NEW_PACKAGING`; the source-instantiated declarations are `FORMALIZED_GAP`. Canonical divisibility ordering, normalization, choice independence, identification of the free tail with an independently constructed historical unbounded `ζ` module, determinant-norm/resultant identification, stage resultants, and primitive normalization remain central gaps; this is not ordinary `Polynomial.resultant`. |
| mutual divisibility and vanishing, Satz VIII | module annihilators; principal ideal products and powers; ideal radicals and polynomial evaluation | the opening finite localized Smith-algebra bounds and generator divisibilities are promoted intrinsically, without naming a greatest selected coefficient; conditional radical and zero-set consequences for supplied historical forms are also promoted. Identification with primitive `R^(i)`/`E^(i)`, later stages, and equations (33)--(34) remain open |
