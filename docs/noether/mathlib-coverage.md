...
Bearbeitung. Its 27 claim/definition records are mapped in
...
| generic transforms and ground ideals, Definitions IV--V and Satz VI | polynomial substitution, localization, relative coefficient modules | equation (12), its natural algebraically-independent lower-pair parameters, equation (17), literal late-variable stage submonoids, simultaneous denominator presentation, coefficient reconstruction, and the final transformed-contraction equality are promoted. Later regularity determinants remain `GAP_CANDIDATE` |
| finite module reduction, Satz VII | finite-free and Smith-normal-form APIs; monic polynomial division; polynomial function extensionality and finite products | equation (21)'s exact bounded-representative theorem and the characteristic-zero simultaneous specialization preserving a supplied finite family of regular leading coefficients are promoted; construction of the `C^(i)`, the multistage finite-module reduction, determinant data, nonunit elementary-divisor theorem, and auxiliary-choice independence remain `GAP_CANDIDATE` |
