...
## *Gleichungen mit vorgeschriebener Gruppe* (1918)

The activated ED0020 witness was read from lines 7882–8084 and split into ten
claims. The green local module deliberately covers only the full symmetric
group baseline: it does not silently promote an invariant-ring theorem into
Noether's arbitrary-subgroup fixed-field or parameterization theorem.

| Source item | Mathlib coverage | State |
|---|---|---|
| Elementary-symmetric algebraic independence, lines 7938–7942 | `MvPolynomial.esymmAlgHom_injective`; `algebraicIndependent_iff_injective_aeval` | local `elementarySymmetric_algebraicIndependent`; `MATHLIB_MODERN_FORM` support, green |
| Full symmetric invariant ring, lines 7935–7942 | exact `MvPolynomial.esymmAlgEquiv` | local source-facing `symmetricInvariantRingAlgEquiv`; `MATHLIB_MODERN_FORM`, green |
| Fraction ring of the full symmetric invariant ring, lines 7938–7942 | `IsFractionRing.algEquivOfAlgEquiv` | local `symmetricInvariantFractionRingAlgEquiv`; `NEW_PACKAGING`, green |
| Equality with the fixed subfield of the ambient rational-function field, lines 7938–7940 | fixed-point subfields and fraction-ring APIs are ingredients only | `GAP_CANDIDATE`; the green abstract fraction-ring equivalence is not this theorem |
| Main minimal-basis and prescribed-group parameterization theorem, lines 7910–7924, 7942–7982, 8015–8017 | polynomial Galois groups, algebraic independence, and intermediate fields are strict ingredients | `DEFERRED_INFRASTRUCTURE`; requires the fixed-field bridge, exact Galois-group control, specialization, and Hilbert irreducibility |
| Possible singular-parameter locus, lines 7930 and 7984–8015 | evaluation, localization, discriminant, and resultant substrate | `GAP_CANDIDATE`; source proves only a one-sided containment and has separate `A₃/A₄` exceptions |
| Reduction from `n` variables to `n - 2`, lines 8024–8067 | elementary symmetric, rename, and homogeneous-polynomial APIs | `GAP_CANDIDATE`; translation, projective ratios, invertibility hypotheses, and exceptional charts remain |
| Degree-three Lüroth step, line 8069 | exact `RatFunc.Luroth.eq_adjoin_generator` and `RatFunc.Luroth.algEquiv` | `MATHLIB_EXACT` central ingredient; transport from Noether's reduction remains open |
| Degree-four Castelnuovo and explicit group bases, lines 8069–8079 | no pinned two-variable Castelnuovo theorem identified | `GAP_CANDIDATE`; direct Klein-four/cubic/quadratic route is also open |
| Abelian groups over a cyclotomic coefficient field, line 8081 | `IsCyclotomicExtension`, `CyclotomicField`, and fixed-point infrastructure | `GAP_CANDIDATE`; action, character-field hypotheses, diagonalization, and rationality proof remain |

The [bounded module receipt](../../artifacts/build/MathematicalCommons-Noether-EquationsWithPrescribedGroup1918-20260829T2315159464772-244c1dd8.module.receipt.json)
is exit zero with a 1,170,911,232-byte peak and only `propext`,
`Classical.choice`, and `Quot.sound`. The Noether and repository umbrellas are
also green below 2 GiB. The exact ten-claim boundary, external API lead, and
source witness are pinned in the
[claim receipt](../../artifacts/build/claim-P11-prescribed-group-20260829T2315159464772-244c1dd8.json).
No German change was made and no P11 transcription defect was identified.
...
Bearbeitung. Its machine-readable coverage records are mapped in
[hentzelt-polynomial-ideals-resultants-1923.md](hentzelt-polynomial-ideals-resultants-1923.md).

| Source layer | Pinned coverage | State |
|---|---|---|
| ground modules, module quotients, and transition norms, Definitions I--III and Sätze I--V | submodule saturation, module torsion and rank-nullity; finite-free modules, Smith normal form, determinant norms, colon ideals, scalar-multiplication preimages | Definition I's formula-(2) cancellation and formula-(4) saturation, Satz I's least-ground-module construction and same-rank consequence, and both reciprocal operations of Satz II are promoted for the actual localized cutoff-one pair. Under the explicit hypothesis that every selected Smith coefficient divides `eᵢ`, `M₁*/G₁* = (eᵢ)` as a scalar colon ideal and `G₁* = M₁*/(eᵢ)` as a module-valued ideal quotient. The broad Satz-II record remains `GAP_CANDIDATE` because Mathlib's selected coefficients are not proved divisibility ordered or canonically normalized; the exact historical divisor phrasing, transition norm, and reconstruction theorems in Definitions III and Sätze III--V remain gaps. |
...
| finite module reduction, Satz VII | finite-free and Smith-normal-form APIs; monic polynomial division; submodule sums and disjointness; `LinearMap.quotientInfEquivSupQuotient`; module torsion and rank-nullity | equations (21)--(23) are promoted for a supplied regular divisor: exact bounded representatives, finite coefficient vectors, the bounded-part/principal-tail decomposition, finite free quotient, exact containing-ideal image, the abstract common-tail quotient, its concrete regular-pair specialization, and the cutoff-one ground-ideal quotient counterpart. The first `ξ`/`ζ` coordinate model realizes the cutoff-one pair and connects its coordinate quotient to the transported ideal quotient and bounded model. Line 13162 is `FORMALIZED_GAP`: `G₁*` is exactly the nonzero-scalar saturation of `M₁*`; the relative quotient is torsion, the denominator has equal rank and `finrank`, and the bounded module is finite. The source-faithful late-variable localization to `K[x₂]` and the actual finite localized Smith pair are now constructed. Later stages and auxiliary-choice independence remain `GAP_CANDIDATE`. |
| equation (24), stage resultants, and elementary-divisor forms, Satz VII and Definition VI | Smith bases and coefficients; `Submodule.quotientEquivPiSpan`; `Submodule.colon`; module annihilators; finite ideal products and powers; determinants; polynomial localization | The actual finite localized cutoff-one pair over `K[x₂]` has selected Smith bases, coefficients, diagonal relation, cyclic quotient decomposition, and selected-basis determinant product. A freely adjoined arbitrary Finsupp common tail, including the countable `ζ` specialization, has unit tail coefficients and cancels exactly back to that finite quotient. The displayed module slashes are formalized as scalar colon ideals: adjoining one `ηᵢ` gives `(eᵢ)` unconditionally, while quotienting by the whole ground module is the infimum of the remaining principal ideals and becomes one `(eᵢ)` only under an explicit greatest-remaining-divisor hypothesis. The finite quotient annihilator is identified intrinsically with `⋂ᵢ(eᵢ)` and with the denominator colon. For `D = ∏ᵢeᵢ` and `A` that annihilator, `(D) ≤ A` and `A^ρ ≤ (D)`; the principal generators satisfy both divisibilities and have equal radicals. Under an explicit `IsGreatestRemainingDivisor` hypothesis, the bridge proves `A = (eᵢ)`, `eᵢ ∣ D`, `D ∣ eᵢ^ρ`, and the associated radical and vanishing statements for the actual localized pair. Localization surjectivity supplies a nonzero integral numerator for that selected coefficient. Given the regular member used in the finite reduction, exact coordinate reconstruction and regular division lift its bounded action to every `g ∈ stageGroundIdeal 1 I`, with a nonzero denominator free of the first two variables. The resulting lift satisfies the full `HasEquation33Witness I 1`. Four further declarations clear the localized coefficient-to-product divisibility, prove the selected-coefficient product nonzero, and construct a compatible nonzero integral product numerator divisible by the coefficient numerator. `FirstSmithPrimitiveCoefficientProductForm` then removes both contents, preserves association after localization, and gives primitive nonzero representatives `e` and `r` satisfying `e ∣ r`, `r ∣ e ^ localizedSecondVariableSmithRank`, and both equation-(33) witnesses. Its generic content/primitive-part helpers are `NEW_PACKAGING`; its source-shaped declarations are `FORMALIZED_SOURCE_PACKAGING`. The companion `FirstSmithPrimitiveTransitionDeterminantForm` adds five declarations: it defines the localized ground-to-denominator transition determinant, proves the selected Smith-basis determinant/product equality, proves association under a basis change, and associates the primitive product proxy with that selected determinant after localization. These remain conditional selected proxies, not the historical `E^(2)` or `R^(2)`, module norm, gcd of maximal minors, or a historical resultant. `DeterminantalIdealScaffold` adds ten source-neutral `NEW_PACKAGING` declarations: selected `k`-minors are submatrix determinants, their ideal is the span of all selected minors, generators belong, repeated rows/columns vanish, all-zero minors give bottom, the positive-degree zero matrix gives bottom, and degree zero gives top. No native Mathlib determinantal/Fitting-ideal API was found. Cauchy--Binet or exterior-power coefficient invariance, identification with a Fitting ideal or maximal-minor gcd, and the historical norm/resultant bridge are not claimed. Existence of a greatest index, canonical ordering, choice independence, historical-tail identification, determinant-norm/resultant identification, and later stage resultants remain central gaps; this is not ordinary `Polynomial.resultant`. |
| mutual divisibility, denominator descent, and vanishing, Satz VIII | module annihilators; principal ideal products and powers; ideal radicals and evaluation; localization contraction; colon ideals; Noetherian finite generation | The intrinsic opening bounds and conditional greatest-coefficient bridge are promoted. `FirstSmithEquation33Bridge` proves that the actual selected greatest cutoff-one coefficient has a nonzero integral numerator whose full-ring lift satisfies `HasEquation33Witness I 1`: regular division handles every member of the first ground ideal and the returned multiplier lies in the literal cutoff-two submonoid. At local cutoff `1`, which is source stage `i = 2`, `FirstSmithCoefficientProductNumerator` produces a compatible integral numerator for the finite selected-coefficient product and preserves the witness under multiplication. `FirstSmithPrimitiveCoefficientProductForm` converts the two selected numerators to primitive integral proxies while retaining association, the integral divisibilities `e ∣ r` and `r ∣ e ^ ρ`, and both equation-(33) witnesses. It does not identify them with historical `E^(2)` or `R^(2)`, a determinant, norm, minor gcd, or resultant, and proves neither canonicity nor choice independence. Equation (33)'s generic one-stage and Noetherian common-multiplier theorems are also promoted. For a supplied consecutive witness family, the library proves arbitrary finite-window iteration, the terminal identity `g_n = I`, the conditional `E`-product half of equation (34), and its tail-window endpoint. Conditional radical and zero-set consequences are also promoted. Later-stage witnesses are still supplied rather than constructed, and the parallel historical `R^(i)` product and determinant-norm/resultant identification remain open. |
...
P22 now accounts for 576 canonical declarations across the base and 38 support
modules. The integrated local graph has 53 direct Noether imports and both
fresh serial umbrella receipts are green. The latest sealed graph contains all
55 targets in the
...
## *Nichtkommutative Algebren* (1933): P40 inner automorphisms

Noether's “Satz über innere Automorphismen” at controlled lines 19477–19483
states that isomorphic finite-rank simple subrings of `A_f` containing the
center are carried into one another by an inner automorphism; in particular,
every automorphism of `A_f` is inner. The
[four-file external audit](../../artifacts/coordination/noether-external-four-file-audit-20260829.json)
read and pinned the actual contents of the relevant TauCeti, QICLean, and
Blackfeather files.

| Source layer | Pinned coverage | State |
|---|---|---|
| Every automorphism of a full matrix algebra is inner, line 19483 | `AlgEquiv.eq_linearEquivConjAlgEquiv` supplies the endomorphism-algebra theorem; QICLean supplied an Apache-2.0 matrix transport wrapper | local `matrixAlgEquiv_inner`, generalized to an arbitrary field; `MATHLIB_MODERN_FORM` plus `NEW_PACKAGING`; green bounded build |
| Conjugacy of two copies of `ℂ` in `ℍ[ℝ]`, lines 19477–19483 | TauCeti's Apache-2.0 quaternion file gives the explicit noncentral-source proof | local `Quaternion.exists_unit_conj_complexAlgHom` and its support declarations; attributed `FORMALIZED_GAP`; green bounded build |
| General central-source Skolem–Noether | TauCeti exports a broad theorem using its tensor-product, bimodule, and simple-Artinian infrastructure | fully inspected dependency model; cluster not copied |
| Arbitrary finite-rank simple subrings containing the center, lines 19477–19483 | the central-source theorem does not cover every printed noncentral source, and the quaternion theorem covers only one concrete case | `GAP_CANDIDATE`; full P40 theorem remains open |
| Jacobson–Noether theorem | Blackfeather's standalone incubator was merged and generalized in `Mathlib.FieldTheory.JacobsonNoether` | `JacobsonNoether.exists_separable_and_not_isCentral`, `MATHLIB_EXACT`; adjacent theorem, not P40 itself |

The attributed adaptations and source boundaries are documented in
[`NoncommutativeAlgebras1933.lean`](../../MathematicalCommons/Noether/NoncommutativeAlgebras1933.lean).
Its [bounded receipt](../../artifacts/build/MathematicalCommons-Noether-NoncommutativeAlgebras1933-20260829T2236562973288-8ad42b68.module.receipt.json)
is green and includes the explicit axiom audit. This does not license a claim
that the printed general theorem has been formalized.
...
- The 1916 invariant-system/degree-bound theorem, 1918 variational theorems,
  ...
  reduction, arbitrary-order discriminant theorem, later ideal theory, and the
  full P40 simple-subring inner-automorphism theorem remain major development
  lanes.
